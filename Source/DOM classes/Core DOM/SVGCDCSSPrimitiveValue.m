...
@implementation SVGCDCSSPrimitiveValue
...
- (id)init
{
    self = [super initWithUnitType:SVGCD_CSS_PRIMITIVE_VALUE];
    if (self) {
...
-(float) getFloatValue:(SVGCDCSSPrimitiveType) unitType
{
	...
	switch( self.primitiveType )
	{
		...
		{
			if( unitType ==     SVGCD_CSS_PX ) /** Dom 1 spec allows this, SVG Spec says "this is correct by spec", and DOM 2 spec says this is illegal. Most CSS interpreters do it... */
			{
				return self.internalValue;
			}
			else
...
-(void)setCssText:(NSString *)newCssText
{
	...
	|| _cssText.length == 0 )
	{
		self.internalValue = 0.0f;
		self.internalString = @"";
		self.primitiveType = SVGCD_CSS_UNKNOWN;
	}
	else if( [_cssText hasSuffix:@"%"])
		[self setFloatValue:    SVGCD_CSS_PERCENTAGE floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"em"])
		[self setFloatValue:    SVGCD_CSS_EMS floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"ex"])
		[self setFloatValue:    SVGCD_CSS_EXS floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"px"])
		[self setFloatValue:    SVGCD_CSS_PX floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"cm"])
		[self setFloatValue:    SVGCD_CSS_CM floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"mm"])
		[self setFloatValue:    SVGCD_CSS_MM floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"in"])
		[self setFloatValue:SVGCD_CSS_IN floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"pt"])
		[self setFloatValue:SVGCD_CSS_PT floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"pc"])
		[self setFloatValue:SVGCD_CSS_PC floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"deg"])
		[self setFloatValue:SVGCD_CSS_DEG floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"rad"])
		[self setFloatValue:SVGCD_CSS_RAD floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"grad"])
		[self setFloatValue:SVGCD_CSS_GRAD floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"ms"])
		[self setFloatValue:SVGCD_CSS_MS floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"s"])
		[self setFloatValue:SVGCD_CSS_S floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"khz"]) // -----------NB: check this before checking HZ !
		[self setFloatValue:SVGCD_CSS_KHZ floatValue:[_cssText floatValue]];
	else if( [_cssText hasSuffix:@"hz"])
		[self setFloatValue:SVGCD_CSS_HZ floatValue:[_cssText floatValue]];
	else
	{
		/**
		...
		if( [scanner scanFloat:&floatToHoldTheOutput])
		{
			...
			[self setFloatValue:    SVGCD_CSS_NUMBER floatValue:floatToHoldTheOutput];
		}
		else
		{
...
#endif
			[self setStringValue:SVGCD_CSS_STRING stringValue:_cssText]; // -------- NB: we allow any string-to-string conversion, so it's not a huge problem that we dont correctly detect "url" versus "other kind of string". I hate CSS Parsing...
		}
	}
}
