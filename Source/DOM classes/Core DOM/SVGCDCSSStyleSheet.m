...
@implementation SVGCDCSSStyleSheet
...
-(long)insertRule:(NSString *)rule index:(unsigned long)index
{
	...
	{
		SVGCDCSSStyleRule* newRule = [[SVGCDCSSStyleRule alloc] initWithSelectorText:[stringSplitContainer objectAtIndex:0] styleText:[stringSplitContainer objectAtIndex:1]];
...
- (id)initWithString:(NSString*) styleSheetBody
{
    ...
    if (self)
	{
		self.cssRules = [[SVGCDCSSRuleList alloc]init];
