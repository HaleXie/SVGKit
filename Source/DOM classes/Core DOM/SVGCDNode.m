...
#import "SVGCDNamedNodeMap_Iterable.h" // Needed for the optional (non-SVG spec) "recursive toXML" method

@implementation SVGCDNode
...
- (id)initType:(SVGCDDOMNodeType) nt name:(NSString*) n value:(NSString*) v
{
    ...
    if (self) {
        ...
        switch( nt )
		{
			...
			case SVGCD_DOMNodeType_ATTRIBUTE_NODE:
			case SVGCD_DOMNodeType_CDATA_SECTION_NODE:
			case SVGCD_DOMNodeType_COMMENT_NODE:
			case SVGCD_DOMNodeType_PROCESSING_INSTRUCTION_NODE:
			case SVGCD_DOMNodeType_TEXT_NODE:
			{
				self.nodeName = n;
				self.nodeValue = v;
			}break;
			...
			case SVGCD_DOMNodeType_DOCUMENT_NODE:
			case SVGCD_DOMNodeType_DOCUMENT_TYPE_NODE:
			case SVGCD_DOMNodeType_DOCUMENT_FRAGMENT_NODE:
			case SVGCD_DOMNodeType_ENTITY_REFERENCE_NODE:
			case SVGCD_DOMNodeType_ENTITY_NODE:
			case SVGCD_DOMNodeType_NOTATION_NODE:
			case SVGCD_DOMNodeType_ELEMENT_NODE:
			{
		...
		self.childNodes = [[SVGCDNodeList alloc] init];
    }
    return self;
}
    ...
    if (self) {
        ...
        switch( nt )
		{
			...
			case SVGCD_DOMNodeType_ELEMENT_NODE:
			{
				...
				self.attributes = [[SVGCDNamedNodeMap alloc] init];
			}break;
		}
...
{
	if( newChild.nodeType == SVGCD_DOMNodeType_DOCUMENT_FRAGMENT_NODE )
	{
		/** Spec:
		...
		for( SVGCDNode* child in newChild.childNodes.internalArray )
		{
...
-(NSString *)textContent
{
	switch( self.nodeType )
	{
		...
		case SVGCD_DOMNodeType_DOCUMENT_FRAGMENT_NODE:
		{
			...
			for( SVGCDNode* subNode in self.childNodes.internalArray )
			{
...
//	NSAssert(namespaceShortnames != nil, @"Must supply an empty dictionary for me to fill with encountered namespaces, and the shortnames I invented for them!");
	...
	{
		SVGCDNode* attribute = [xmlnsNodemap objectForKey:xmlnsNodeName];
	...
	for( NSString* namespace in nodeMapsByNamespace )
	{
		...
		{
			SVGCDNode* attribute = [nodeMap objectForKey:nodeNameFromMap];
	...
	switch( self.nodeType )
	{
		...
		case SVGCD_DOMNodeType_ELEMENT_NODE:
		{
			for( SVGCDNode* child in self.childNodes )
			{
