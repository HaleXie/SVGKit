	...
	for (size_t n = 0; n <= len; n++) {
		...
		if (c == ':') {
            ...
            if( accumIdx > 0 ) //if there is a ';' and '\0' to end the style, avoid adding an empty key-value pair
            {
				...
				SVGCDCSSValue *cssValue;
				if( [cssValueString rangeOfString:@" "].length > 0 )
					cssValue = [[SVGCDCSSValueList alloc] init];
				else
					cssValue = [[SVGCDCSSPrimitiveValue alloc] init];
...
{
	SVGCDCSSValue* v = [self getPropertyCSSValue:propertyName];
...
-(SVGCDCSSValue*) getPropertyCSSValue:(NSString*) propertyName
{
...
-(NSString*) item:(long) index
{
	...
	SVGCDCSSValue* v = [sortedKeys objectAtIndex:index];
	return v.cssText;
}
