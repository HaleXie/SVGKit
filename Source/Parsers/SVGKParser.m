        ...
        if( startHref.location != NSNotFound )
        {
            ...
            if( startHref.location != NSNotFound )
            {
                ...
                if( cssSource != nil )
                {
                    ...
                    SVGCDCSSStyleSheet* parsedStylesheet = [[SVGCDCSSStyleSheet alloc] initWithString:cssText];
...
- (void)handleStartElement:(NSString *)name namePrefix:(NSString*)prefix namespaceURI:(NSString*) XMLNSURI attributeObjects:(NSMutableDictionary *) attributeObjects
{
	...
	if( ! parsingRootTag && _storedChars.length > 0 )
	{
		...
		SVGCDText *tNode = [[SVGCDText alloc] initWithValue:_storedChars];
	...
	for( NSObject<SVGKParserExtension>* subParser in self.parserExtensions )
	{
		   ...
		   && [[subParser supportedTags] containsObject:name] )
		{
			...
			SVGCDNode* subParserResult = [subParser handleStartElement:name document:source namePrefix:prefix namespaceURI:XMLNSURI attributes:attributeObjects parseResult:self.currentParseRun parentNode:_parentOfCurrentNode];
	...
	/** Parser Extenstion creates a node for us */
	SVGCDNode* subParserResult = [eventualParser handleStartElement:name document:source namePrefix:prefix namespaceURI:XMLNSURI attributes:attributeObjects parseResult:self.currentParseRun parentNode:_parentOfCurrentNode];
...
static void startElementSAX (void *ctx, const xmlChar *localname, const xmlChar *prefix,
	...
	for( SVGCDAttr* newAttribute in attributeObjects.allValues )
	{
	...
	for( NSString* prefix in namespacesByPrefix )
	{
		 ...
		 */
		SVGCDAttr* newAttributeFromNamespaceDeclaration = [[SVGCDAttr alloc] initWithNamespace:@"http://www.w3.org/2000/xmlns/" qualifiedName:[NSString stringWithFormat:@"xmlns:%@", prefix] value:namespace];
...
static NSMutableDictionary *NSDictionaryFromLibxmlAttributes (const xmlChar **attrs, int attr_ct) {
	...
	for (int i = 0; i < attr_ct * 5; i += 5) {
		...
		SVGCDAttr* newAttribute = [[SVGCDAttr alloc] initWithNamespace:uri qualifiedName:qname value:value];
...
+(NSDictionary *) NSDictionaryFromCSSAttributes: (SVGCDAttr*) styleAttribute {
	...
	for (size_t n = 0; n <= len; n++) {
		...
		if (c == ':') {
			...
			SVGCDAttr* newAttribute = [[SVGCDAttr alloc] initWithNamespace:styleAttribute.namespaceURI qualifiedName:[NSString stringWithUTF8String:name] value:[NSString stringWithUTF8String:accum]];
